...
function Categories() {
    ...
    return (
        <>
            <div className='flex flex-col justify-start items-center h-[80vh] overflow-y-scroll mb-[1rem]'>
                <div>
                            ...
                            <div className='flex md:flex-row flex-col justify-center flex-wrap gap-[2rem]'>
                                {filteredBlogs.map((blog) =>
                                    <Link to={`/blog/${blog?._id}`}>
                                        <FeaturedImage className="object-cover rounded-[20px] md:w-[35rem] w-[20rem] h-[15rem] md:h-[22rem]" category={blog.category} img={`https://mern-blog-app-8l09.onrender.com/images/${blog?.photo}`} title={blog?.title} desc={blog?.desc} author={blog?.userId?.name} likes={blog?.likes.length} views={blog?.views} />
                                    </Link>
                                )}
                            </div>
